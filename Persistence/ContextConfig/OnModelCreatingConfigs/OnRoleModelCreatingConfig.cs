...
namespace Persistence.ContextConfig.OnModelCreatingConfigs
{
    public class OnRoleModelCreatingConfig : IEntityTypeConfiguration<Role>
    {
        public void Configure(EntityTypeBuilder<Role> builder)
        {
            ...
            builder.HasData(new Role
            {
                Id = RoleEnum.Karzar.GetId(),
                Name = Enum.GetName(RoleEnum.Karzar),
                NormalizedName = Enum.GetName(RoleEnum.Karzar).Normalize(),
                DisplayName = RoleEnum.Karzar.GetDescription()
            });
        }
    }
}